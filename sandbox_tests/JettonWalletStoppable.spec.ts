...
describe("JettonWallet", () => {
  ...
  it("if premint is true, minter should mint jettons to to_address first", async () => {
    let premintResult = await jettonMinter.sendMint(
      deployer.getSender(),
      deployer.address,
      toNano("100"),
      toNano("1"),
    ...
    expect(premintResult.transactions).toHaveTransaction({
      from: deployer.address,
      to: jettonMinter.address,
      deploy: true,
    });
    // prettyLogTransactions(premintResult.transactions);
    // printTransactionFees(premintResult.transactions);
    // console.log("except");
    expect(await jettonMinter.getTotalSupply()).toEqual(toNano("100"));
    // check that minter has minted jettons to deployer
  ...
  it("minter admin should be able to mint jettons", async () => {
    ...
    expect(await deployerJettonWallet.getJettonBalance()).toEqual(
      initialJettonBalance + toNano("100")
    );
  ...
  it("minter admin can change admin", async () => {
    ...
    expect(
      (await jettonMinter.getAdminAddress()).equals(deployer.address)
    ).toBe(true);
  });
  ...
  it("minter admin can change content", async () => {
    let newContent = jettonContentToCell({
      type: 1,
  ...
  it("wallet owner should be able to send jettons", async () => {
    ...
    let initialTotalSupply = await jettonMinter.getTotalSupply();
    const notDeployerJettonWallet = await userWallet(notDeployer.address);
    let initialJettonBalance2 =
      await notDeployerJettonWallet.getJettonBalance();
  ...
  it("works with minimal ton amount", async () => {
    ...
    let tonBalance2 = (await blockchain.getContract(someJettonWallet.address))
      .balance;
    ...
    expect(await someJettonWallet.getJettonBalance()).toEqual(
      initialJettonBalance2 + sentAmount
    );

    tonBalance = (await blockchain.getContract(deployerJettonWallet.address))
      .balance;
    expect(
      (await blockchain.getContract(someJettonWallet.address)).balance
    ).toBeGreaterThan(min_tons_for_storage);
  });
...
*/
  ...
  describe("Bounces", () => {
    ...
    it("wallet should restore balance on burn_notification bounce", async () => {
      ...
      const burnMsg = JettonWallet.burnMessage(
        burnAmount,
        deployer.address,
        null
      );
  ...
  it.skip("owner can withdraw excesses", async () => {
    ...
    let initialBalance = (await blockchain.getContract(deployer.address))
      .balance;
    const withdrawResult = await deployerJettonWallet.sendWithdrawTons(
      deployer.getSender()
    );
  ...
  it.skip("owner can withdraw jettons owned by JettonWallet", async () => {
    ...
    expect(
      (await deployerJettonWallet.getJettonBalance()) - initialJettonBalance
    ).toEqual(toNano("0.4"));
  ...
  it("admin can send stop message", async () => {
    const sendResult = await jettonMinter.sendStop(deployer.getSender());
    expect(sendResult.transactions).toHaveTransaction({
      from: deployer.address,
      to: jettonMinter.address,
      success: true,
  ...
  it("if minter is stopped, it should not accept mint messages", async () => {
    const sendResult = await jettonMinter.sendStop(deployer.getSender());
    ...
    expect(res.transactions).toHaveTransaction({
      ...
      to: jettonMinter.address,
      aborted: true,
      exitCode: Errors.stopped,
    });
  });

  it("if minter is stopped, it should not accept change_owner messages", async () => {
    const sendResult = await jettonMinter.sendStop(deployer.getSender());
    const res = await blockchain.sendMessage(
      internal({
        from: deployer.address,
        to: jettonMinter.address,
        body: JettonMinter.changeAdminMessage(notDeployer.address),
        value: toNano("0.1"),
      })
    );
  ...
  it("if minter is stopped, it should not accept discovery messages", async () => {
    const sendResult = await jettonMinter.sendStop(deployer.getSender());
    const res = await blockchain.sendMessage(
      internal({
        from: deployer.address,
        to: jettonMinter.address,
        body: JettonMinter.discoveryMessage(notDeployer.address, true),
        value: toNano("0.1"),
      })
    );
