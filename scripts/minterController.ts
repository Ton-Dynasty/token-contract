...
const mintAction = async (provider: NetworkProvider, ui: UIProvider) => {
  ...
  do {
    retry = false;
    const fallbackAddr =
      sender.address ?? (await minterContract.getAdminAddress());
    mintAddress = await promptAddress(
      `Please specify address to mint to`,
      ui,
      fallbackAddr
    );
    mintAmount = await promptAmount(
      "Please provide mint amount in decimal form:",
      ui
    );
...
export async function run(provider: NetworkProvider) {
  ...
  do {
    ...
    if (
      ...
      const stateCode = Cell.fromBoc(
        Buffer.from(curState.account.state.code, "base64")
      )[0];
  ...
  const isAdmin = hasSender
    ? (await minterContract.getAdminAddress()).equals(sender.address)
    : true;
